import time, sys, os, re, threading, queue, zmq, socket, errno, syslog
...
class Monitor(threading.Thread):
	def zmqRequester(self,flag,jail,action,attacker): #this is where the magic begins
		if jail=='fail2bancluster': return True #ignore fail2bancluster bans
		syslog.syslog("Propagating: %s for %s in %s" % (action,attacker,jail))
		zmqReplyContext = zmq.Context()
		zmqReplySock = zmqReplyContext.socket(zmq.REQ)
		zmqReplySock.connect(self.zmqReplyServer)
		#TODO: sanitize jail names, action (Ban/Unban), and attacker (IP address regex)
		#TODO: also sanitize hostname according to RFC 1123
		outmsg=""
		if self.authenticate=="true": outmsg=self.authtoken+"|"
...
#			syslog.syslog('notifier got item: ',flag,jail,action,attacker)
	...
	def __init__ (self,monitorconfig=None,num_worker_threads=1):
		...
		# I call it "ReplyServer" because it is a zeromq REQUEST/REPLY type of socket
		...
		#Prepare regex
		self.regex = re.compile(".*\[(.*)\]\ (Ban|Un[bB]an)\ (.*)") #once upon a time Unban and UnBan happened #FIXME
	...
	def run(self):
		...
		while not self._stopevent.isSet():
			...
			if not line:
				if os.stat(self.logfilename)[ST_SIZE] < where: # logfile truncated or rotated. we got to reset.
					syslog.syslog("fail2ban-zmq-cluster Monitor: fail2ban logfile rotation detected.")
					self.logfile.close()
					self.logfile=open(self.logfilename,'r')
					where=self.logfile.tell()
				else:
			...
			else: # match the line. attempt to extract jail (postfix, apache-badbots, etc), action (Ban/UnBan) and IP
