...
class Subscriber(threading.Thread):
    def __init__(self, subscriberconfig=None, num_worker_threads=1):
        ...
        # we want to ignore our own messages.
    ...
    def run(self):
        ...
        while not self._stopevent.isSet():
            ...
            # TODO: INPUT CHECK HERE - apply regex and such against message
            # parts (jail, ip, etc) TODO: act according to publisheraction
            # [see fail2ban-cluster.conf]
            msg = message.split('|')
            Hostname = msg[0]
            Jail = msg[1]
            Action = msg[2]
            ...
            if not f2bcUtils.is_valid_hostname(Hostname):
                syslog.syslog("fail2ban-zmq-tools Subscriber: \
                               Invalid hostname in incoming message.")
                continue
            # If hostname matches our hostname, output warning, using
            # different syntax to avoid triggering the fail2bancluster jail
            # filter.
            if Hostname == self.hostname:
                syslog.syslog("fail2ban-zmq-tools Subscriber: \
                               Got equal hostname broadcast. \
                               Our hostname is %s" % self.hostname)
                continue
